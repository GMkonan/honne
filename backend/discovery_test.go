...
func TestSearchAniListNormalizesAndCaches(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"data":{"Page":{"pageInfo":{"hasNextPage":false},"media":[{"id":1,"siteUrl":"https://anilist.co/anime/1","format":"TV","description":"Space bounty hunters.<br>In space.","episodes":26,"averageScore":88,"title":{"romaji":"Cowboy Bebop","english":"Cowboy Bebop","native":"カウボーイビバップ"},"coverImage":{"extraLarge":"https://example.com/cover.jpg"},"startDate":{"year":1998},"studios":{"nodes":[{"name":"Sunrise"}]}}]}}}`))
	}))
	defer server.Close()
	...
	for range 2 {
		...
		if len(response.Results) != 1 || response.Results[0].Total != 26 || response.Results[0].Subtitle != "Sunrise" || strings.Contains(response.Results[0].Description, "<br>") {
...
func TestSearchBooksNormalizesCover(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"numFound":1,"docs":[{"key":"/works/OL123W","title":"Dune","author_name":["Frank Herbert"],"first_publish_year":1965,"cover_i":42,"number_of_pages_median":412}]}`))
	}))
	defer server.Close()
	...
	if result.ProviderID != "OL123W" || result.Total != 412 || !strings.Contains(result.CoverURL, "/42-L.jpg") {
...
func TestSearchTMDBUsesBearerToken(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"total_pages":1,"results":[{"id":550,"title":"Fight Club","original_title":"Fight Club","overview":"An insomniac.","poster_path":"/poster.jpg","release_date":"1999-10-15","vote_average":8.4}]}`))
	}))
	defer server.Close()
	...
	result := response.Results[0]
	if result.ReleaseYear != 1999 || result.ProviderID != "550" || result.Type != "movie" {
		t.Fatalf("unexpected normalized result: %+v", result)
	}
}
...
func TestAniListImportMapsAndPersistsEntries(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"data":{"User":{"name":"konan","avatar":{"large":"https://example.com/avatar.jpg"}},"anime":{"lists":[{"entries":[{"id":700,"status":"CURRENT","score":8,"progress":12,"notes":"great","media":{"id":20,"siteUrl":"https://anilist.co/anime/20","format":"TV","description":"Ninjas.<br>More ninjas.","episodes":220,"title":{"romaji":"NARUTO","english":"Naruto","native":"NARUTO -ナルト-"},"coverImage":{"extraLarge":"https://example.com/naruto.jpg"},"startDate":{"year":2002},"studios":{"nodes":[{"name":"Pierrot"}]}}}]}]},"manga":{"lists":[{"entries":[{"id":701,"status":"PLANNING","score":0,"progress":0,"notes":"","media":{"id":123,"siteUrl":"https://anilist.co/manga/123","format":"NOVEL","description":"A novel.","chapters":10,"title":{"romaji":"Novel","english":"A Novel","native":"小説"},"coverImage":{"extraLarge":"https://example.com/novel.jpg"},"startDate":{"year":2020},"studios":{"nodes":[]}}}]}]}}}`))
	}))
	defer server.Close()
